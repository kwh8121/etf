...
- **회사 PC 근무시간 제약(2026-09-22 확정)**: KR dispatch는 09:00~17:00 KST의 평일에만 허용하여 최대 60분 workflow가 18:00 이전에 끝나도록 한다. PC가 꺼져 있으면 다음 근무시간까지 보고가 지연된다. US는 무손실 필수 범위에서 제외했으며 현 07:30 timer 재편은 별도 작업이다. 설치본은 원본 파일 변경만으로 갱신되지 않으므로 아래 설치 명령과 실제 발화 확인이 필요하다.

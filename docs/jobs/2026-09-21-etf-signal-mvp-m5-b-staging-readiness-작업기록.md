...
- GitHub CLI 인증 점검: 기존 토큰이 무효여서 Actions 실행 이력·staging Environment 설정을 조회하거나 수동 실행할 수 없었다.
...
1. GitHub 인증을 복구한 뒤 `staging` Environment에 별도 Supabase·Telegram 테스트 수신처·Kiwoom 자격증명과 `ENABLE_US_ETF_P1=true`가 있는지 값 없이 존재 여부만 확인한다.
2. `us-etf-movers.yml`을 수동으로 2회 실행해 복합 관측 중복·단일 US `signal_run`·P1 Telegram 1건을 확인한다.
3. `ENABLE_US_ETF_P1=false`로 되돌린 뒤 disabled 스모크와 국내 P0 비간섭을 기록한다.

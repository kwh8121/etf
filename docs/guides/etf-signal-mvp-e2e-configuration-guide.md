...
- KR 보충은 과거 KRX 일별 데이터와 파생 신호만 복구하며, 과거 시점의 Kiwoom 마스터·신규 상장 관측이나 당시 Telegram 보고는 재현하지 않는다. 새 보고 자동 발송도 하지 않는다. 휴장일로 판단된 평일은 매번 KRX를 재조회한다. 이미 완료한 뒤 나중에 앞선 거래일이 채워지면 뒤 날짜의 신호 기간도 다시 계산한다. 이는 운영 E2E 검증 전의 구현 경계다.

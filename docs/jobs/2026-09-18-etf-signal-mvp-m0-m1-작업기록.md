...
>
> 상태: 기능·DB 기반 구축 완료 / production build 재검증 대기
...
다만 `npm run build`는 코드 오류가 아니라 현재 실행 환경의 Google Fonts 네트워크 제한 및 Turbopack 프로세스·포트 권한 제약으로 다시 확인해야 한다. 따라서 기능·DB 기반은 완료지만 production build 품질 게이트는 미해결 상태로 기록한다.
...
1. 네트워크·프로세스 권한이 갖춰진 환경에서 `npm run build`를 재실행한다.
2. M2에서 KRX/Kiwoom 수집 결과를 원천 snapshot과 정규화 테이블에 저장한다.
3. 같은 대상일의 재실행이 중복 저장 없이 동작하는지 검증한다.

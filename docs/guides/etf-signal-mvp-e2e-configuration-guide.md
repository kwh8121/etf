...
| timer                          | 시각                                    | 실행                                                         |
| ------------------------------ | --------------------------------------- | ------------------------------------------------------------ |
| `etf-kr-daily-dispatch.timer`  | 월~금 09:30 KST                         | `kr-daily.yml`, 입력 없이 2026-09-22부터 전일의 누락일 보충 |
| `etf-us-movers-dispatch.timer` | 월~금 09:40 KST (직전 미국 거래일 장 마감 후) | `us-etf-movers.yml`                                      |

- `Persistent=true`: 놓친 timer 발화는 다음 기동 때 한 번만 따라잡는다. KR workflow는 매번 2026-09-22부터 전일까지 모든 평일을 순서대로 점검하므로 여러 발화를 놓쳐도 KRX 일별 행과 신호 누락을 재시도한다. 실패한 날짜에서 멈추고 다음 실행에서 재시도한다. 기존 `TRADING_COMPLETE` KRX 행은 재수집하지 않고, `NON_TRADING` 평일은 일시적 빈 응답 가능성 때문에 다시 확인한다.
- **회사 PC 근무시간 제약(2026-09-22 확정)**: KR dispatch는 09:00~17:00 KST의 평일에만 허용하여 최대 60분 workflow가 18:00 이전에 끝나도록 한다. PC가 꺼져 있으면 다음 근무시간까지 보고가 지연된다. US는 무손실 필수 범위에서 제외했으며, 평일 09:40 KST에 직전 미국 거래일 장 마감 결과를 처리한다. 설치본은 원본 파일 변경만으로 갱신되지 않으므로 아래 설치 명령과 실제 발화 확인이 필요하다.
- KR 보충은 과거 KRX 일별 데이터와 파생 신호만 복구하며, 과거 시점의 Kiwoom 마스터·신규 상장 관측이나 당시 Telegram 보고는 재현하지 않는다. 단 보충이 막힘 없이 끝나면 마지막 거래일 1건은 Kiwoom 마스터 동기화(관측일=실행일)·신호 재생성·운영 Telegram 보고까지 수행한다(2026-09-23 Q-10 A안). 발송 성공 뒤 해당 `signal_run.notes.telegram_reported_at`을 남기며, 같은 기준일에 표식이 있으면 다시 보내지 않는다. 발송 실패 시 표식이 없어 다음 실행에서 재시도한다. 실행일 아침 동기화에서 처음 관측된 신규 상장은 다음 거래일 보고에 포함된다. 같은 입력의 신호 재생성도 표식을 보존하며, 수동 `daily:kr` 발송도 같은 표식을 남긴다. 표식 확인과 기록은 원자적이지 않으므로 workflow `concurrency` 밖에서 운영 DB 대상 `catchup:kr`를 로컬로 동시에 실행하지 않는다. 휴장일로 판단된 평일은 매번 KRX를 재조회한다. 이미 완료한 뒤 나중에 앞선 거래일이 채워지면 뒤 날짜의 신호 기간도 다시 계산한다. 이는 운영 E2E 검증 전의 구현 경계다.

...
describe("KRX new listing signals", () => {
  it("emits only unalerted events that appeared after the initial master snapshot", () => {
    const result = createKrxNewListingSignals([
      {
        ...
        newInSnapshot: false,
        firstAlertedAt: null,
        sourceSnapshotId: "snapshot-initial",
      },
      {
        eventKey: "event-alerted",
        code: "ALERTED",
        name: "Alerted",
        regDay: "2026-09-17",
        firstSeen: "2026-09-17",
        newInSnapshot: true,
        firstAlertedAt: "2026-09-17T10:00:00Z",
        sourceSnapshotId: "snapshot-alerted",
      },
      {
        eventKey: "event-new",
        code: "NEW",
    ...
    expect(result.signals).toEqual([
      expect.objectContaining({ code: "NEW", value: 1, screen: "raw" }),
    ]);
    expect(result.sourceSnapshotIds).toEqual(["snapshot-new"]);
    expect(result.metaByCode.get("NEW")).toMatchObject({
      event_key: "event-new",
      reg_day: "2026-09-18",
      new_in_snapshot: true,
    });
  });
});
